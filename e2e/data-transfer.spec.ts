 ...
 */
test("history survives an export, a wiped browser store and an import", async ({ page }) => {
  // A session to have something to lose.
  await page.goto(`/skriv?mode=passage&work=ibsen-brand&segment=${segment.id}&filter=as-printed`);
  ...
  expect(download.suggestedFilename()).toMatch(/^brand-data-\d{4}-\d{2}-\d{2}\.json$/);
  ...
  await expect(page.getByTestId("transfer-status")).toContainText("1 økter lest inn");
  await expect(page.getByTestId("history-table")).toBeVisible();
  expect(await page.getByTestId("history-table").locator("tbody tr").count()).toBe(rowsBefore);
});

test("a file that is not an export is refused, and nothing is lost", async ({ page }) => {
  await page.goto("/historikk");
